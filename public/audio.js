$(document).ready(_ => {
    ...
    const dbname = 'godb-xlll';
    const db = new Firebase(`https://${dbname}.firebaseio.com`);
    const storage = db.child('music');
    const get = new URLSearchParams(window.location.search);
    const list = [];
    $('.control').each((i, self) => {
        list.push($(self).data('src'))
    });
    let index = 0;
    ...
    const loadData = () => {
        $.ajax({
            type: 'GET',
            url: `https://${dbname}.firebaseio.com/music.json`,
    ...
    const findCover = (artist, title) => {
        ...
        $.get(`https://ws.audioscrobbler.com/2.0/?method=track.getInfo&api_key=${key}&artist=${artist}&track=${title}&format=json`).done((data) => {
            $.each(data, (i, v) => {
    ...
    const reverse = (el) => {
        list.reverse();
        return el.each((i, self) => {
            let $this = $(self);
            $this.children().each((i, self) => {
                $this.prepend($(self))
            })
        })
    };
    ...
    const playPause = (index, initial = index) => {
        let cover,
            curSrc = player.currentSrc.replace(/.+[\\\/]/, ''),
    ...
    const next = () => {
        ...
        player.src = list[index];
        let cover = $('.audio').eq(index).find('.audio-btn').css('background-image').replace(/^url\(['"](.+)['"]\)/, '$1');
        cover === 'none' && (
            cover = 'assets/img/cover.png'
        ),
        player.play()
    ...
    $('.volume').on('input', () => {
        ...
        $('.fill').css('height', ($('.volume').val() * 10 * 4) + 1),
        player.volume === 0 ? (
            $('.mute').addClass('off')
        ) : (
    ...
    $(document).on('click', '.repeat', () => {
        $('.repeat').toggleClass('loop')
    }),

    $(document).on('click', '.reverse', () => {
    ...
    $(document).on('click', '.song_modal', () => {
        ...
        $('.player .repeat').hasClass('loop') && $('.player_full .repeat').addClass('loop'),
        $('.player_full .progress').css('width', $('.player .progress').width())
    }),
    ...
    $(document).on('click', '#add', (e) => {
        ...
        artist ? title ? url ? checkUrl(url) ? list.indexOf(url) === -1 ? cover ? checkImg(cover) ? (
            ...
            $('#add_artist, #add_title, #add_url, #add_cover').val(''),
            $('#result').empty()
        )
    ...
    $(document).mouseup((e) => {
        let $target = $(e.target);
        $target.closest('.search_btn').length === 0 && $target.closest('.search').length === 0 && $('.search').removeClass('active')
    }),

    $(document).keyup('#search-input', () => {
        let searchTerm = $('#search-input').val(),
            searchSplit = searchTerm.replace(/ /g, ' ');
        $.extend($.expr[':'], {
            containsi: (elem, i, match) => (elem.textContent || elem.innerText || '').toLowerCase().indexOf((match[3] || '').toLowerCase()) >= 0
        }),
        $('.playlist .audio').not(`:containsi("${searchSplit}")`).each((i, self) => {
            $(self).addClass('notfound').removeClass('found'),
            $('.notfound').addClass('hidden')
        }),
        $(`.playlist .audio:containsi("${searchSplit}")`).each((i, self) => {
            $(self).removeClass('notfound').addClass('found').removeClass('hidden'),
    ...
    $(window).scroll(_ => {
        ...
        scroll > scrollPrev ? $('.player').addClass('hide') : $('.player').removeClass('hide'),
